...
class KpiController extends Controller
{
    public function getGeneration($gen_id)
    {
        $generation = Generation::where('id', $gen_id)->first();
        return response()->json($generation);
    }

    public function getCurrentGenUsers()
    {
        ...
        $users = User::whereHas('group', function ($query) use ($currentPeriod) {
            $query->where('gen_id', $currentPeriod->gen_id)
                  ->whereNotIn('id', [99, 100, 101, 102]); // Exclude specific groups
        })->with('group')->get();
    ...
    {
        $minimumValues = MinimumValue::first(); // Assuming only one row exists
        return response()->json($minimumValues);
    }

    public function store(Request $request)
    {
        $kpiData = $request->input('kpis');

        foreach ($kpiData as $data) {
            Kpi::create([
                'user_id' => $data['user_id'],
                'no_room' => $data['no_room'],
                'kedisiplinan_point' => $data['kedisiplinan_point'],
                'kedisiplinan_nilai' => $data['kedisiplinan_nilai'],
                'kesehatan_point' => $data['kesehatan_point'],
                'kesehatan_nilai' => $data['kesehatan_nilai'],
                'safety_point' => $data['safety_point'],
                'safety_nilai' => $data['safety_nilai'],
                'r5_point' => $data['r5_point'],
                'r5_nilai' => $data['r5_nilai'],
                'vt7_point' => $data['vt7_point'],
                'vt7_nilai' => $data['vt7_nilai'],
